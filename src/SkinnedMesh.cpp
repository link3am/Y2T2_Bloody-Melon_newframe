...
void GLTFSkinnedMesh::LoadAnimation(int AnimIndex)
{
	tinygltf::Animation& animation = m_model.animations[AnimIndex];
	for (int i = 0; i < animation.channels.size();i++) {
		samplerData temp;
		tinygltf::AnimationSampler& sampler = animation.samplers[animation.channels[i].sampler];
		if (animation.channels[i].target_path=="rotation") {
			temp.KetFrameData = RetrieveModelData<glm::vec4>(sampler.output,TINYGLTF_TYPE_VEC4);
		}
		else {
			auto KeyFrameDate = RetrieveModelData<glm::vec3>(sampler.output, TINYGLTF_TYPE_VEC3);
			for (int j = 0; j < KeyFrameDate.size(); j++) {
				temp.KetFrameData.push_back(glm::vec4 (KeyFrameDate[j].x, KeyFrameDate[j].y, KeyFrameDate[j].z,0));
			}
		}
		temp.KeyFrameTime = RetrieveModelData <float>(sampler.input,TINYGLTF_TYPE_SCALAR);
		anim.push_back (temp);
	}
}
...
template<typename T>
T GLTFSkinnedMesh::SampleAnimationChannel(tinygltf::AnimationChannel& const channel, float time, tinygltf::Animation& const animation,  int channelIndex, int const accessorType)
{
	...
	std::vector<float> keyframeTimes = anim[channelIndex].KeyFrameTime;
...
//		frame1 = glm::vec3(anim[channelIndex].KetFrameData[frame1Index]);
//		frame2 = glm::vec3(anim[channelIndex].KetFrameData[frame2Index]);
//	}
//	else {
//		frame1 = anim[channelIndex].KetFrameData[frame1Index];
//		frame2 = anim[channelIndex].KetFrameData[frame2Index];
//	}
...
template<>
glm::vec3 GLTFSkinnedMesh::SampleAnimationChannel(tinygltf::AnimationChannel& const channel, float time, tinygltf::Animation& const animation, int channelIndex, int const accessorType)
{
	tinygltf::AnimationSampler& sampler = animation.samplers[channel.sampler];
...
template<>
glm::vec4 GLTFSkinnedMesh::SampleAnimationChannel(tinygltf::AnimationChannel& const channel, float time, tinygltf::Animation& const animation, int channelIndex, int const accessorType)
{
	tinygltf::AnimationSampler& sampler = animation.samplers[channel.sampler];
...
void GLTFSkinnedMesh::Draw(Shader::sptr& const shader, glm::mat4& const viewProjection, const glm::mat4& worldTransform)
{
...
float GLTFSkinnedMesh::GetAnimationLength(tinygltf::Animation& const animation)
{
	...
	for (int i = 0; i < anim.size(); i++) {
		float time = anim[i].KeyFrameTime[anim[i].KeyFrameTime.size() - 1];
		if (time > length)
		{
			length = time;
		}
	...
	for (int i = 0; i < animation.channels.size(); i++)
	{
		...
		if (channel.target_path == "translation")
		{
			glm::vec3 translation =
				SampleAnimationChannel<glm::vec3>(channel, m_animationTime, animation,i, TINYGLTF_TYPE_VEC3);
			transform->m_position = translation;
		}
		else if (channel.target_path == "rotation")
		{
			glm::vec4 rotation =
				SampleAnimationChannel<glm::vec4>(channel, m_animationTime, animation,i, TINYGLTF_TYPE_VEC4);
