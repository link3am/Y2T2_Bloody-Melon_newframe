...
struct samplerData {
public:
    std::vector <glm::vec4> KetFrameData;
...
public:
    ...
    void Draw(Shader::sptr& const shader, glm::mat4& const viewProjection, const glm::mat4& worldTransform);

    template<typename T>
    T SampleAnimationChannel(tinygltf::AnimationChannel& const channel, float time,
        tinygltf::Animation& const animation,int channelIndex, int const accessorType = TINYGLTF_TYPE_VEC3);

    template<>
    glm::vec3 SampleAnimationChannel(tinygltf::AnimationChannel& const channel, float time,
        tinygltf::Animation& const animation, int channelIndex, int const accessorType);

    template<>
    glm::vec4 SampleAnimationChannel(tinygltf::AnimationChannel& const channel, float time,
        tinygltf::Animation& const animation, int channelIndex, int const accessorType);
...
private:
    ...
    void LoadAnimation(int AnimIndex);

    std::vector <samplerData> anim;
